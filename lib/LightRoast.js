 ...
 */
var LightRoast = function LightRoast(name, config) {
  var isRoot = config.isRoot;

  this._Commander = require('commander');
  this._commander = new this._Commander.Command();

  // The executable associated with these commands
  this.executable = (isRoot ? name : '');
  ...
  // To be an array after parsing the arguments that is accessible to people
  // using this command
  this.args = [];
  ...
  if (this.arguments.length && this.subcommands.length) {
    throw new Error(
  ...
  this._hasSubcommands = !!this.subcommands.length;
...
LightRoast.prototype._recursivelyBuildCommands = function _recursivelyBuildCommands(config) {
  ...
  for (name in config.subcommands) {
    if (config.subcommands.hasOwnProperty(name)) {
      this.subcommands.push(new LightRoast(name, config.subcommands[name]));
    }
  }
};

/**
 * Set the executable name for help output purposes.  This needs to be passed
 * from parent to child the same way the command list is.
 */
LightRoast.prototype.setExecutableName = function setExecutableName(executableName) {
...
LightRoast.prototype.parse = function parse(argv, callback) {
  ...
  // Decide whether or not to output the help at this level, regardless of
  // whether this is a parent or child command
  if (needsHelp) {
    if (cmd.args.length === this.level) {
  ...
  // command
  subcommandKeyword = this._commander.args[this.level];
  this.subcommands.every(function (instance) {
...
LightRoast.prototype._generateHelp = function _generateHelp() {
  ...
  var argumentHelp = [];
  var optionsHelp = [];
  var usage = '$> ' + this.executable + ' [options]' +
  ...
  if (!this._hasSubcommands && this.options.length) {
    this._help.push('');
...
LightRoast.prototype._parseArgs = function _parseArgs(args) {
  var argsValid = true;
  var requiredArgCount = this.arguments.reduce(function (prev, curr) {
     return prev + (curr.required ? 1 : 0);
  }, 0);

  if (args.length !== this.level + requiredArgCount) {
    console.log(
      '\n' +
      '\u001b[31m' +
  ...
  args = args.slice(this.level);

  // Validate the arguments
  this.arguments.every(function (argSpec, idx) {
